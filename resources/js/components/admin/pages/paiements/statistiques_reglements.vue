<template>
    <div class="main-panel">
        <div v-if="$can('Consulter les statistiques des réglements encaissés')" class="content-wrapper">
 
              <!-- note -->
         <div class="modal fade details_reglements" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                        ...
                        <div class="col-md-12">
                           <div class="d-flex justify-content-between">
                              <h3 class="card-title mb-0"> 
                                 Détails des réglements encaissés par jour
                              </h3>
                           </div>
                        </div>
                     </div>
                  </div>
                  <div ref="details_reglementsContainer" class="modal-body grid-margin modal-utilisateur ">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <h1 class="card-title">
                                            ...
                                            <span class="titre"> Statistiques des règlements encaissés </span>
                                        </h1><br><br>
                                    </div>
                                </div>
                                <div class="row">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="form-group mr-b-0em">
                                                    <label class="form__label">ID / Cin</label>
                                                     <input v-model.trim="value_id_adherent" placeholder="ID ou Cin" class="form-control">
                                                </div>
                                            </div>
                                                <div class="col-md-4">
                                                <div class="form-group mr-b-0em">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="card-header ">
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <h3>Tableau des réglements encaissés</h3>
                                                </div>
                                            </div>
                                        </div><br>
                                            ...
                                            <div style="overflow-x:auto;" class="tab-pane fade in active show">
                                                    ...
                                                    <tbody>
                                                        <tr v-for="(sts, index) in filteredStatistiquesReglements">
                                                            <td> 
                                                                ...
                                                                <span class="months_clicable" title="Détials des réglements encaissés" data-toggle="modal" data-target=".details_reglements" @click="OpenModelDetailsReglements(sts.monthKey, sts.year, 0 , index )" > {{ options_months[parseInt(sts.monthKey - 1)].name  }}   </span>
                                                ...
                                                <div class="rows">
                                                    <div class="col-md-6">
                                                        <div class="d-flex align-items-center pb-2">
                                                            <div class="dot-indicator bg-dot-primary mr-2"></div>
                                                            <p class="mb-0 dot-font-size">Total</p>
                                                        </div>
                                                        <h4 class="font-weight-semibold font-size-semibold">{{montantTotal}} {{$t('content.text.dh')}}</h4>
                                                        <div class="progress progress-md">
                                                            <div class="progress-bar bg-dot-primary" role="progressbar" style="width: 100%"></div>
                                                        </div>
                                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="card-header ">
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <h3>Graphique des réglements encaissés</h3>
                                                </div>
                                            </div>
                                        </div>
                                        <div><br>
...
export default {
    ...
    methods: {
        ...
        StatistiquesReglements(data_filtrer) {
            ...
            if (this.$v.$invalid) {
                ...
                console.log(this.data_filtrer.datetimes[0])
                console.log(this.data_filtrer.datetimes[1])
                ...
                axios.post('/api-admin/statistiques-reglements', this.formData, config).then((response) => {
                    ...
                    if (typeof response.data.adherent !== "undefined") {
                        ...
                        for (var i = 0; i < response.data.paiements.length; i++) {
                            ...
                            this.charts.months.push(response.data.paiements[i].months)
                            this.charts.sums.push(parseFloat(response.data.paiements[i].sums))
                            this.charts.background.push(this.arr_background[i])
                            this.montantTotal =this.montantTotal + parseFloat(parseFloat(response.data.paiements[i].sums));
                        }
                        this.statistiques_reglements = response.data.paiements
                        this.loaded = true
                    }
        ...
        OpenModelDetailsReglements(month, year, day, index){
            this.month = month
            this.year = year

            this.popup_search = ''
            if (typeof this.$refs.details_reglementsContainer !== "undefined")
                    this.PluginLoader(false, this.$refs.details_reglementsContainer)
                this.formData = new FormData();
                ...
                this.details_reglements.splice(0)
                ...
                axios.post('/api-admin/statistiques-reglements', this.formData, config).then((response) => {
                    if (typeof response.data.adherent !== "undefined") {
                        ...
                        for (var i = 0; i < response.data.paiements.length; i++) {  
                            this.montantTotalMonth =this.montantTotalMonth + parseFloat(parseFloat(response.data.paiements[i].sums));
                        }
                        this.details_reglements = response.data.paiements
                      }                    
                });
        }
    },
    beforeMount() {
        if(this.$can('Consulter les statistiques des réglements encaissés')){
    ...
    computed: {
        ...
        filteredStatistiquesReglementsMonth() {
            return this.details_reglements.filter(reglement => {
                return reglement.dayKey.toLowerCase().includes(this.popup_search.toLowerCase())
            })
        },
    }
}
