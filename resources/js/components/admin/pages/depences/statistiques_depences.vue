<template>
    <div class="main-panel">
        <div v-if="$can('Consulter les statistiques des dépenses')" class="content-wrapper">
                <!-- note -->
         <div class="modal fade details_depenses_month" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                        ...
                        <div class="col-md-12">
                           <div class="d-flex justify-content-between">
                              <h3 class="card-title mb-0"> 
                                 Détails des dépenses par jour
                              </h3>
                           </div>
                        </div>
                     </div>
                  </div>
                  <div ref="details_depenses_monthContainer" class="modal-body grid-margin modal-utilisateur ">
                    <div class="row mt-3 mb-3">
                        <div class="col-md-4">
                            <p>
                                <b>Dépenses : </b><span>  {{ nom_service }} </span>
                            </p>
                        </div>
                        <div class="col-md-4">
                            <p>
                     ...
                     <div  class="mt-3 responsive-table " >
                        ...
                        <table class="mt-2 datatables table table-striped table-bordered table-hover">
                           ...
                           <tbody>
                              <tr v-for="(paiement, index) in filteredStatistiquesDepencesMonth"  >
                                 <td>
                            ...
                            <div class="tab-content">
                                <div class="row">
                                    <div class="col-md-12">
                                        <h1 class="card-title">
                                            ...
                                            <span class="titre"> Statistique des dépenses  </span>
                                        </h1><br><br>
                                    </div>
                                </div>
                                <div class="row">
                                ...
                                <div class="row">
                                   <div class="col-6">
                                        ...
                                        <div v-if="statistiques_depences.length > 0">
                                            <div style="overflow-x:auto;" class="tab-pane fade in active show">
                                                <table class="datatables table table-striped table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        <tr v-for="(sts, index) in statistiques_depences">
                                                            <td> {{ sts.nom_service }} </td>
                                                            <!--<td> {{ sts.qte }} </td>-->
                                                            <td>
                                                                
                                                                <span class="months_clicable" title="Détials paiements" data-toggle="modal" data-target=".details_depenses_month" @click="OpenModelDetailsDepensesMonth(sts.monthKey, sts.year, 0 , index , sts.nom_service)" > {{ options_months[parseInt(sts.monthKey - 1)].name  }}  </span>
...
<script>
 export default {
    ...
    methods: { 
        ...
        StatistiquesDepences(data_filtrer) {
                ...
                axios.post('/api-admin/dashboard/statistiques-depences' , this.formData, config).then((response) => {
                    ...
                    this.statistiques_depences = response.data
                    this.loaded = true
                 });
        ...
        filtrer(event) {
            ...
            this.StatistiquesDepences(this.data_filtrer)
        },
        ...
        OpenModelDetailsDepensesMonth(month, year, day, index, nom_service){
            this.month = month
            this.year = year
            this.nom_service = nom_service
 
            this.popup_search = ''
            if (typeof this.$refs.details_depenses_monthContainer !== "undefined")
                    this.PluginLoader(false, this.$refs.details_depenses_monthContainer)
                this.formData = new FormData();
                ...
                this.details_depenses_month.splice(0)
                ...
                axios.post('/api-admin/dashboard/statistiques-depences', this.formData, config).then((response) => {
                    ...
                    this.details_depenses_month = response.data
    ...
    beforeMount() { 
         this.ListServicesTypeDepences()
        this.filtrer();
        var dt = new Date();
        for (var startYear = 2020; startYear <= dt.getFullYear(); startYear++) {
            this.options_years.push({ name: startYear, year_id: startYear })
        }
    },
    computed: {
        filteredStatistiquesDepences() {
            return this.statistiques_depences.filter(reglement => {
                //return reglement.months.toLowerCase().includes(this.search_months.toLowerCase())
            })
        },
        filteredStatistiquesDepencesMonth() {
            return this.details_depenses_month.filter(depense => {
                return depense.dayKey.toLowerCase().includes(this.popup_search.toLowerCase())
            })
        },
    }
}
